...
import {
  ...
  MoonKeySessionKeyPlugin__factory,
  MoonKeySessionKeyPlugin
} from '../typechain'
import {
  createAddress,
  createAccountOwner,
...
describe.only('MoonKey Gnosis Proxy', function () {
  this.timeout(30000)

  let ethersSigner: Signer
  let safeSingleton: MoonKeyPluginSafe
  let owner: Signer
  ...
  let accountFactory: MoonKeyGnosisSafeAccountFactory
  ...
  before('before', async function () {
    ...
    safeSingleton = await new MoonKeyPluginSafe__factory(ethersSigner).deploy(entryPoint.address)
    ...
    accountFactory = await new MoonKeyGnosisSafeAccountFactory__factory(ethersSigner)
    ...
    proxy = MoonKeyPluginSafe__factory.connect(addr, owner)
  ...
  describe('policy session key', function () {
    ...
    let policyPlugin: MoonKeySessionKeyPlugin
    beforeEach(async function () {
      ...
      policyPlugin = await new MoonKeySessionKeyPlugin__factory(ethersSigner).deploy()
...
async function getSessionNonce (
  plugin: MoonKeySessionKeyPlugin,
  sender: string,
  sessionKey: Signer
): Promise<number> {
  return await MoonKeyPluginSafe__factory.connect(sender, plugin.provider).callStatic
