 ...
 */
struct MoonKeySessionKeyStorageStruct {
    mapping(address => bool) revoked;
    mapping(address => uint256) sessionNonce;
}

contract MoonKeySessionKeyPlugin is MoonKeyBasePlugin {
    ...
    constructor() EIP712("MoonKeySessionKeyPlugin", "1.0.0") {
    }

    function getPolicyStorage() internal pure returns (MoonKeySessionKeyStorageStruct storage s) {
        bytes32 position = bytes32(uint256(keccak256("moon-key.account.eip4337.sessionkey")) - 1);
        assembly {
            s.slot := position
        }
    }
